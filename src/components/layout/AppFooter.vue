<template>
  <footer class="bg-purple-950 text-white">
    <!-- Main Footer -->
    <div class="container-custom py-16">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        <!-- Brand -->
        <div class="lg:col-span-1">
          <img src="@/assets/logo.jpeg" alt="Formation Exceptionelle" class="h-14 w-auto mb-4 brightness-0 invert" />
          <p class="text-purple-300 text-sm leading-relaxed mb-6">
            Your partner in career development. Empowering professionals across Africa and beyond with world-class training, mentorship, and opportunities.
          </p>
          <div class="flex items-center gap-3">
            <a
              v-for="social in socials"
              :key="social.name"
              :href="social.url"
              target="_blank"
              class="w-10 h-10 rounded-full bg-purple-800 hover:bg-purple-600 flex items-center justify-center transition-colors"
              :aria-label="social.name"
            >
              <span class="w-5 h-5 flex items-center justify-center" v-html="social.icon"></span>
            </a>
          </div>
        </div>

        <!-- Services -->
        <div>
          <h4 class="font-bold text-lg mb-5 text-gold-400">Our Services</h4>
          <ul class="space-y-3">
            <li v-for="item in serviceLinks" :key="item.label">
              <RouterLink
                :to="item.path"
                class="text-purple-300 hover:text-white text-sm transition-colors flex items-center gap-2"
              >
                <ChevronRightIcon class="w-3 h-3 text-gold-400" />
                {{ item.label }}
              </RouterLink>
            </li>
          </ul>
        </div>

        <!-- Quick Links -->
        <div>
          <h4 class="font-bold text-lg mb-5 text-gold-400">Quick Links</h4>
          <ul class="space-y-3">
            <li v-for="item in quickLinks" :key="item.label">
              <RouterLink
                :to="item.path"
                class="text-purple-300 hover:text-white text-sm transition-colors flex items-center gap-2"
              >
                <ChevronRightIcon class="w-3 h-3 text-gold-400" />
                {{ item.label }}
              </RouterLink>
            </li>
          </ul>
        </div>

        <!-- Contact & Newsletter -->
        <div>
          <h4 class="font-bold text-lg mb-5 text-gold-400">Stay Connected</h4>
          <div class="space-y-3 mb-6">
            <div class="flex items-start gap-3 text-sm text-purple-300">
              <MapPinIcon class="w-4 h-4 text-gold-400 mt-0.5 flex-shrink-0" />
              <span>123 Victoria Island, Lagos, Nigeria</span>
            </div>
            <div class="flex items-center gap-3 text-sm text-purple-300">
              <EnvelopeIcon class="w-4 h-4 text-gold-400 flex-shrink-0" />
              <span>info@formationexceptionelle.com</span>
            </div>
            <div class="flex items-center gap-3 text-sm text-purple-300">
              <PhoneIcon class="w-4 h-4 text-gold-400 flex-shrink-0" />
              <span>+234 800 000 0000</span>
            </div>
          </div>

          <!-- Newsletter -->
          <p class="text-sm text-purple-300 mb-3">Subscribe to our newsletter</p>
          <form @submit.prevent="handleNewsletter" class="flex gap-2">
            <input
              v-model="email"
              type="email"
              placeholder="Your email"
              class="flex-1 px-3 py-2 rounded-lg bg-purple-800/50 border border-purple-700 text-white placeholder-purple-400 text-sm focus:outline-none focus:border-gold-400"
            />
            <button
              type="submit"
              class="px-4 py-2 bg-gold-500 hover:bg-gold-400 rounded-lg text-white text-sm font-semibold transition-colors"
            >
              Join
            </button>
          </form>
        </div>
      </div>
    </div>

    <!-- Bottom Bar -->
    <div class="border-t border-purple-800">
      <div class="container-custom py-5 flex flex-col md:flex-row items-center justify-between gap-4">
        <p class="text-purple-400 text-sm text-center">
          &copy; {{ currentYear }} Formation Exceptionelle. All rights reserved.
        </p>
        <div class="flex items-center gap-6">
          <a href="#" class="text-purple-400 hover:text-white text-sm transition-colors">Privacy Policy</a>
          <a href="#" class="text-purple-400 hover:text-white text-sm transition-colors">Terms of Service</a>
          <a href="#" class="text-purple-400 hover:text-white text-sm transition-colors">Cookie Policy</a>
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup>
import { ref } from 'vue'
import { RouterLink } from 'vue-router'
import { toast } from 'vue3-toastify'
import {
  ChevronRightIcon,
  MapPinIcon,
  EnvelopeIcon,
  PhoneIcon,
} from '@heroicons/vue/24/outline'

const currentYear = new Date().getFullYear()
const email = ref('')

const socials = [
  {
    name: 'LinkedIn', url: '#',
    icon: `<svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>`
  },
  {
    name: 'Twitter / X', url: '#',
    icon: `<svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.253 5.622 5.91-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>`
  },
  {
    name: 'Facebook', url: '#',
    icon: `<svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>`
  },
  {
    name: 'Instagram', url: '#',
    icon: `<svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>`
  },
]

const serviceLinks = [
  { label: 'Online Courses (LMS)', path: '/lms' },
  { label: 'Job Opportunities', path: '/jobs' },
  { label: 'Internship Programs', path: '/jobs' },
  { label: 'Become an Instructor', path: '/become-instructor' },
  { label: 'Corporate Training', path: '/#services' },
]

const quickLinks = [
  { label: 'About Us', path: '/#about' },
  { label: 'Our Faculty', path: '/#faculty' },
  { label: 'Success Stories', path: '/#testimonials' },
  { label: 'Sign In', path: '/auth/login' },
  { label: 'Create Account', path: '/auth/register' },
]

function handleNewsletter() {
  if (!email.value) return
  toast.success('Thank you for subscribing!')
  email.value = ''
}
</script>
