<template>
  <AdminLayout>
    <div class="max-w-3xl">
      <div class="mb-8">
        <RouterLink to="/admin/jobs" class="flex items-center gap-2 text-purple-700 font-medium text-sm mb-4 hover:gap-3 transition-all">
          <ArrowLeftIcon class="w-4 h-4" /> Back to Job Management
        </RouterLink>
        <h1 class="text-2xl font-bold text-gray-900 mb-1">{{ isEditing ? 'Edit Job Posting' : 'Post a Job or Internship' }}</h1>
        <p class="text-gray-600">{{ isEditing ? 'Update the details of this listing.' : 'Create a new job or internship listing. It goes live immediately.' }}</p>
      </div>
      ...
      <div v-if="!submitted" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <form @submit.prevent="handlePost" class="space-y-6">
            ...
            <div class="space-y-4">
              ...
              <div class="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input v-model="form.company" type="text" placeholder="Company name" class="input-field" required />
                </div>
                <div>
          ...
          <button type="submit" :disabled="jobsStore.loading" class="btn-primary w-full py-4 text-base">
            ...
            <span v-else class="flex items-center gap-2"><RocketLaunchIcon class="w-5 h-5" /> {{ isEditing ? 'Save Changes' : 'Publish Job' }}</span>
          </button>
        </form>
      </div>
      ...
      <div v-else class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
        ...
        <h2 class="text-2xl font-bold text-gray-900 mb-3">{{ isEditing ? 'Job Updated Successfully!' : 'Job Published!' }}</h2>
        <p class="text-gray-600 mb-8">
          {{ isEditing ? 'Your changes have been saved.' : 'The listing is now live and visible to candidates.' }}
        </p>
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          <RouterLink to="/admin/jobs" class="btn-primary px-8 py-3">Back to Job Management</RouterLink>
          <button v-if="!isEditing" @click="postAnother" class="btn-outline px-8 py-3">Post Another Job</button>
        </div>
      </div>
    </div>
  </AdminLayout>
